...
`sb_delete_event()` uses the Smartabase API (v1) to delete event data in Smartabase. It requires an `event_id` value.
...
`sb_delete_event()` is fairly straightforward. Besides the usual Smartabase credentials, it expects only one argument: `event_id`. These are the IDs of records that are returned by `sb_get_event()` or `sb_synchronise_event()`.
