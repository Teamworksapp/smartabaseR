...
There are two different import modes available to all import functions: `import_mode = "batch"` or `import_mode = "single"`. By default, events will be imported in batch mode, which means that all events will be imported in a single API call. Typically this is faster but will provide less detailed error messages if something goes wrong. 

Setting `import_mode = "single"` will import each event in a separate API call. This is slower but will provide more detailed error messages if something goes wrong.

#### Batch Mode

For example, let's say we are importing three events in batch mode:

```{r batch_mode_example1, echo = FALSE}
batch_import_data <- tibble::tibble(
  `Sprint` = c(10.2, 9.9, 10.0),
  Status = c(1, 2, NA_character_)
)

batch_import_data2 <- batch_import_data %>% 
  dplyr::mutate(
    user_id = c(11111, 11111, 99999),
    start_date = c("01/01/2023", "01/01/2023", "02/01/2023")
  )

batch_import_data2
```

```{r batch_mode_example3, echo = TRUE, eval = FALSE}
batch_import_response <- sb_insert_event(
  df = batch_import_data,
  form = "Sprint Testing",
  url = "example.smartabase.com/site",
  username = "example.username",
  password = "example_password",
...
```{r, batch_mode_example4, echo = FALSE}
cli::cli_alert_success(
  "SUCCESSFULLY_INSERTED: 3 out of 3 records successfully inserted into \\
  Sprint Testing."
)

batch_import_response <- batch_import_data %>% 
  dplyr::mutate(
    import_action = c("insert", "insert", "insert"),
    import_result = c("SUCCESSFULLY_INSERTED", "SUCCESSFULLY_INSERTED", "SUCCESSFULLY_INSERTED"),
    api_call_index = c(1, 1, 1),
    import_time = c(1722911550000, 1722911550000, 1722911550000),
    entered_by_user_id = c(58012, 58012, 58012),
    start_time = c("9:00 AM", "9:00 AM", "9:00 AM"),
    end_time = c("10:00 AM", "10:00 AM", "10:00 AM"),
    end_date = c("01/01/2023", "01/01/2023", "02/01/2023")
)
```

```{r batch_mode_example5, echo = TRUE}
print(batch_import_response)
```

The `api_call_index` column in the import response shows that all three events were imported in a single API call. 

Now imagine that we made `Status` a required field in the `Sprint Testing` form. If we tried to re-import the same data now, the event with an empty value in the `Status` column will fail on import. In batch mode, that means the entire import will fail, even though two of the three events contain values for the required `Status` field:

```{r batch_mode_example6, echo = FALSE}
cli::cli_alert_warning("UNEXPECTED_ERROR:")

batch_import_response <- batch_import_data %>% 
  dplyr::mutate(
    import_action = c("insert", "insert", "insert"),
    import_result = c("UNEXPECTED_ERROR", "UNEXPECTED_ERROR", "UNEXPECTED_ERROR"),
    api_call_index = c(1, 1, 1),
    import_time = c(NA, NA, NA),
    form = c(NA_character_, NA_character_, NA_character_),
    entered_by_user_id = c(NA_character_, NA_character_, NA_character_),
    start_time = c("9:00 AM", "9:00 AM", "9:00 AM"),
    end_time = c("10:00 AM", "10:00 AM", "10:00 AM"),
    end_date = c("01/01/2023", "01/01/2023", "02/01/2023")
)
...
```{r batch_mode_example7, echo = TRUE}
  print(batch_import_response)
```

#### Single Mode

Ideally, we would like to import the valid events and receive an error log pointing to the invalid events, which we can achieve by setting `import_mode = "single"`:

```{r single_mode_example1, echo = FALSE, eval = FALSE}
single_import_response <- sb_insert_event(
  df = batch_import_data,
  form = "Sprint Testing",
  url = "example.smartabase.com/site",
  username = "example.username",
...
```{r single_mode_example2, echo = TRUE}
cli::cli_alert_success(
  "1: SUCCESSFULLY_INSERTED: 1 out of 1 records successfully inserted into \\
  Sprint Testing."
)
cli::cli_alert_success(
  "2: SUCCESSFULLY_INSERTED: 1 out of 1 records successfully inserted into \\
  Sprint Testing."
)
cli::cli_alert_warning("UNEXPECTED_ERROR:")

single_import_response <- batch_import_data %>% 
  dplyr::mutate(
    import_action = c("insert", "insert", "insert"),
    import_result = c("SUCCESSFULLY_INSERTED", "SUCCESSFULLY_INSERTED", "UNEXPECTED_ERROR"),
    api_call_index = c(1, 1, 2),
    import_time = c(1722911550000, 1722911550000, NA),
    entered_by_user_id = c(58012, 58012, NA),
    form = c("Sprint Testing", "Sprint Testing", NA_character_),
    entered_by_user_id = c(58012, 58012, NA_character_),
    start_time = c("9:00 AM", "9:00 AM", "9:00 AM"),
    end_time = c("10:00 AM", "10:00 AM", "10:00 AM"),
    end_date = c("01/01/2023", "01/01/2023", "02/01/2023")
)
...
```{r single_mode_example3, echo = TRUE}
print(single_import_response)
```

You can see that the first two valid events were successfully imported, while the event with an empty value in the `Status` column failed. The downside of single mode is that each event is imported in a separate API call, which can be slower than batch mode.
