...
#' @param filter More filters accessible via [sb_get_event_filter()]
#' @param option More options accessible via [sb_get_event_option()]
...
#' @param password Smartabase password
#' @param ... These dots are for future extensions and must be empty
#' @param filter More filters accessible via [sb_sync_event_filter()]
#' @param option More options accessible via [sb_sync_event_option()]
#'
#' @returns A tibble containing Smartabase event data
#' @export
#'
...
#' @param password Smartabase password
#' @param ... These dots are for future extensions and must be empty
#' @param option More options generated by a [sb_get_user_option()]
#'
...
#' @param password Smartabase password
#' @param ... These dots are for future extensions and must be empty
#' @param filter More filters generated by a [sb_get_user_filter()]
#' @param option More options generated by a [sb_get_user_option()]
#'
...
sb_get_user <- function(
  ...
  cache_args <- arg[!names(arg) %in% c("login", "current_env")]
  key <- paste0(serialize(cache_args, NULL), collapse = "")
  .sb_cache(
    key = key,
    expr = quote(.export_handler(arg)),
    cache = arg$option[["cache_user"]],
    cache_timeout = arg$option[["cache_user_timeout"]],
    cache_label = "sb_get_user",
...
#'
#' @description
#' Download license audit. Admin users only.
#'
#' @param url Smartabase url e.g. "example.smartabase.com/site"
#' @param username Smartabase username
#' @param password Smartabase password
#' @param ... These dots are for future extensions and must be empty
#' @param option More options generated by [sb_get_license_audit_option()]
...
#' print(license_audit)
#' }
sb_get_license_audit <- function(
    url,
    username,
    password,
    ...,
    option = sb_get_license_audit_option()
  ) {
  rlang::check_dots_used()
  env <- rlang::current_env()
  .check_export_class(filter = NULL, option, env)
  ...
  arg <- list(
    ...
    password = password,
    option = option,
    endpoint = "membership/downloadLicenseAudit",
    endpoint_type = "audit",
    api_version = "v2",
    current_env = env,
    ...
  ...
  .sb_cache(
    key = key,
    expr = quote(.export_handler(arg)),
    cache = arg$option[["cache_audit"]],
    cache_timeout = arg$option[["cache_audit_timeout"]],
    cache_label = "sb_get_license_audit",
    interactive_mode = arg$option$interactive_mode
  )
}
